...
def main():
    ...
    parser.add_argument(
        "-a", "--attempts", type=int, default=64, help="Attempts for each graph"
    )
    parser.add_argument(
        "-d",
        "--data_path",
    ...
    if args.data_path is not None:
        ...
        instances = [
            CSP_Instance.generate_random(args.n_variables, args.n_clauses, language)
            for _ in tqdm(range(args.n_instances))
        ]

    conflicting_edges = evaluate_boosted(
        network, instances, args.t_max, attempts=args.attempts
    )
