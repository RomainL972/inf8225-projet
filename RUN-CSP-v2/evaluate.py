...
def evaluate_boosted(network, eval_instances, t_max, attempts=64):
    ...
    for i, instance in tqdm(enumerate(eval_instances), total=len(eval_instances)):
        start = time.time()
        output_dict = network.predict_boosted(
            instance, iterations=t_max, attempts=attempts
        )
        end = time.time()
    ...
    print(f"solved {solved_problems} out of {len(eval_instances)} problems")
...
def main():
    ...
    parser.add_argument(
        "-m", "--model_dir", type=str, help="Path to the trained RUN-CSP instance"
    )
    parser.add_argument(
        "-v",
        "--n_variables",
    ...
    parser.add_argument(
        "-t", "--t_max", type=int, default=40, help="Number of network iterations t_max"
    )
    parser.add_argument(
        "-a",
        "--attempts",
