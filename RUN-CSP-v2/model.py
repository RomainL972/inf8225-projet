...
class Symmetric_Message_Network:
    ...
    def __call__(self, in_right, in_left):
        ...
        n_edges = tf.shape(input=in_right)[0]
        msg_left = y[:n_edges, :]
        msg_right = y[n_edges:, :]
...
class RUN_CSP_Cell:
    ...
    def __init__(self, network):
        ...
        self.var_state_init = tf.compat.v1.random_normal_initializer()
...
class RUN_CSP:
    ...
    def __init__(self, model_dir, language, state_size=128):
        ...
        self.x_init = tf.compat.v1.zeros_initializer()
    ...
    def build(self):
        ...
        loss = tf.reduce_sum(input_tensor=self.build_loss())
    ...
    def build_loss(self):
        ...
        for r, M in self.relation_tensors.items():
            ...
            relation_loss = tf.reduce_sum(input_tensor=relation_loss, axis=0)
...
class Max_IS_Network(RUN_CSP):
    ...
    def build_loss(self):
        ...
        max_loss = factor * (1.0 - tf.reduce_mean(input_tensor=self.p, axis=0))
