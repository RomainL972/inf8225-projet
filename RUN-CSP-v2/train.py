...
# def train(network, train_data, val_data, t_max, epochs):
#     """
...
#         conflict_ratio_train = output_dict["conflict_ratio"]

#         val_conflict_ratios = []
#         for instance in val_data:
#             output_dict = network.predict(instance, iterations=t_max)
#             val_conflict_ratios.append(output_dict["conflict_ratio"])

#         conflict_ratio_val = (
#             float(np.mean(val_conflict_ratios)) if val_conflict_ratios else 1.0
#         )

#         print(
#             f"Train conflict ratio: {conflict_ratio_train:.4f}, Val conflict ratio: {conflict_ratio_val:.4f}"
#         )

#         # if network improved, save new model
#         if conflict_ratio_val < best_conflict_ratio:
#             network.save_checkpoint("best")
#             best_conflict_ratio = conflict_ratio_val
...
def train(network, train_data, t_max, epochs):
    ...
    for e in range(epochs):
        ...
        print(f"Conflict ratio: {conflict_ratio_train:.4f}")

        # if network improved, save new model
        if conflict_ratio_train < best_conflict_ratio:
            network.save_checkpoint("best")
            best_conflict_ratio = conflict_ratio_train
...
def main():
    ...
    parser.add_argument(
        ...
        "--model_dir",
        type=str,
        help="Path to the model directory where the trained RUN-CSP instance will be stored",
    )
    parser.add_argument(
        "-v",
        "--n_variables",
    ...
    train_instances = [
        CSP_Instance.generate_random(
            args.n_variables, np.random.randint(args.c_min, args.c_max), language
        )
